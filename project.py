...
def deleteRestaurant(restaurant_id):
    ...
    if restaurantToDelete.user_id != login_session['user_id']:
        return """<script>function myFunction(){alert('You are not authorized to delete this restaurant.
                  only the creator of a restaurant is authorized to delete it.');}</script><body onload='myFunction()'>"""
    if request.method == 'POST':
...
def showMenu(restaurant_id):
    ...
    items = session.query(MenuItem).filter_by(restaurant_id=restaurant_id).all()
    # checks username and creator to determine view
    if 'username' not in login_session and creator.id != login_session['user_id']:
        return render_template('publicmenu.hmtl', restaurant=restaurant, creator=creator, items=items)
    else:
        return render_template('menu.html', restaurant=restaurant, creator=creator, items=items)
